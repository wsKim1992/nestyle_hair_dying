...
//이미지 다운받아서 로컬 스토리지에 저장...
	...
	async function saveAndSendCanvas(canvas){
		...
		$("#progressMsgBar").show();
		poolingProcess =await setInterval(sendPoolingMsg('on'),1000);
		xhp = $.ajax({
			url:"/upload_img",
			type:'post',
			cache:false,
			...
			xhr:function(){
				...
				tempXHR.addEventListener("readystatechange",function(event){
					const readyState = tempXHR.readyState;
					const status = tempXHR.status;
					if(readyState>=0&&readyState<2){
						$("#stopAjaxBtn").show();
						$("#progressMsgBar").text("uploading..You can stop uploading now");
						$("#stopAjaxBtn").on("click",async function(e){
							e.preventDefault();
							console.log("readyState : "+xhp.readyState);
							clearInterval(poolingProcess);
							poolingProcess=null;
							sendPoolingMsg('off');
							console.timeEnd('david');
							if(xhp&&xhp.readyState<2){
								xhp.abort();
								$("#loadDyedImg").hide();
								$("#DyedImg").show();
								$("#stopAjaxBtn").hide();
			...
			success:async function(resp){
				clearInterval(poolingProcess);
				poolingProcess=null;
				console.log(poolingProcess);
				$("#stopAjaxBtn").hide();
				hash = resp['hash'];
				$("#stopAjaxBtn").hide();
				const dirPath = "./static/log/"+today_date+'/1_upload/';
				const src = dirPath.toString()+hash+'.png';
				
				let alphaList = resp['alpha'];
				//console.log(alphaList);
				originWidth=parseInt(alphaList.length);
				let newAlphaList = [];
				for(let i=0;i<alphaList.length;i++){
					newAlphaList.push(alphaList[i]);
				}
				alpha = newAlphaList.join(',')
				...
				if(liNum<=6){
					...
					$li.children('img').attr({"src":src,"imgType":"uploaded"}).addClass('on');
	...
	function sendPoolingMsg(msg){
		let params = "msg="+msg;
		let ajaxRequest = new window.XMLHttpRequest();
		ajaxRequest.open("POST","pooling",true);
		ajaxRequest.setRequestHeader('Content-type','application/x-www-form-urlencoded');
		ajaxRequest.responseType='json';
		ajaxRequest.send(params);//get방식일 때는 null

		ajaxRequest.addEventListener("load",function(event){
			if (ajaxRequest.response["msg"]!=msg){$("#progressMsgBar").text(ajaxRequest.response["msg"]);}
		})
		ajaxRequest.addEventListener("readystatechange",function(){
			const readyState = ajaxRequest.readyState;
			if(readyState==4){
				const status = ajaxRequest.status;
				sleep(450);
				if(msg!='off'&&(poolingProcess!=null&&poolingProcess!=undefined)&&status==200){
					sendPoolingMsg(msg);
				}	
			}
		})
	...
	$("#colorList #colorListContainer label").on("click",async function(e){
		...
		if(imgType=='sample'){
			...
			if(poolingProcess!=null&&poolingProcess!=undefined){
				alert("이미 염색이 진행중입니다!");
			}else{
				
				let blobFile=createBloB(croppedImgURL,'image/png');
				const _for = $(this).attr("for");
				
				let a1,b1,a2,b2;
				if(_for=='black'){
					a1=0;b1=30;a2=160;b2=0;
				}else{
					a1=0;b1=128;a2=240;b2=16;
				}
				...
				let formData = new FormData();
				formData.append('file',blobFile,'imgFile');
				formData.append('hash',hash);
				formData.append('alpha',alpha);
				formData.append('today_date',today_date);
				formData.append('target_h',h_value);
				formData.append('a1',a1);
				formData.append('b1',b1);
				formData.append('a2',a2);
				formData.append('b2',b2);
				formData.append('length',originWidth);
				...
				xhp=$.ajax({ 
					...
					success:async function(resp){
						...
						let imgSrcBase64 ='data:image/png;base64,'+resp['img_info_b64'].split('b\'')[1].split('\'')[0];
						
						let dirPath = './static/log/'+today_date+'/3_result/';
						const src = dirPath + resp['filename'];
						await $("#DyedImg").attr({"src":imgSrcBase64});
						await $("#loadDyedImg").hide();
						//$("#downLink").attr({"href":src});
						$("#DyedImg").show();
					},
